...
router.post("/get-key/start", async (req, res) => {
  try {
    ...
    // The target URL is our callback that will verify step 1
...
// Step 2: Verify Step 1 and generate Step 2 EarnPaste link
router.get("/get-key/verify-step1", async (req, res) => {
  ...
  try {
    ...
    if (error || !data) {
      return res.status(400).send("Invalid or expired session step.");
    }

    // Redirect back to the frontend with the session info
    // The frontend will then call the next endpoint to get Step 2 link
...
router.post("/get-key/step2", async (req, res) => {
  ...
  try {
    ...
    if (response.data.url) {
      ...
      throw new Error("Failed to create Step 2 link");
    }
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
// Step 4: Verify Step 2 and generate final key
router.get("/get-key/verify-step2", async (req, res) => {
  ...
  try {
    ...
    // Also insert the key into a 'keys' table for the redeem page to check
    ...
    res.status(500).send("Server error during final verification.");
  }
});

// Endpoint for frontend to fetch the generated key once completed
router.get("/get-key/result/:sessionId", async (req, res) => {
  ...
  try {
    const { data, error } = await supabase
      ...
      .match({ id: sessionId, status: "completed" })
      .single();

    if (error || !data) {
      return res.status(404).json({ error: "Key not found or session not completed" });
    }
...
// Endpoint to verify and redeem a key
