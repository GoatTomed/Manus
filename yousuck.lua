...
export default {
  async fetch(request) {
    ...
    // Fonction pour renvoyer la page d'accès refusé
    const returnAccessDenied = (detectedIp) => {
      return new Response(ACCESS_DENIED_HTML(detectedIp), {
        status: 403,
        headers: {
    ...
    // Fonction pour renvoyer la page de connexion
    const returnLoginPage = (detectedIp, errorMessage = '') => {
      return new Response(LOGIN_HTML(detectedIp, errorMessage), {
        status: 200,
        headers: {
          "Content-Type": "text/html",
          "Cache-Control": "no-store, no-cache, must-revalidate, proxy-revalidate",
    ...
    // Seul le chemin TARGET_PATH est géré
    if (url.pathname !== TARGET_PATH) {
      return returnAccessDenied(ip);
    }
    ...
    // Si l'IP est autorisée, vérifier le mot de passe
    ...
    if (request.method === 'POST') {
      ...
      if (submittedPassword === PASSWORD) {
        isAuthenticated = true;
        // Définir un cookie pour maintenir la session
        const response = new Response(LUA_SCRIPT_CONTENT, {
          headers: {
            "Content-Type": "text/plain",
            "Cache-Control": "no-store, no-cache, must-revalidate, proxy-revalidate",
            "Pragma": "no-cache",
            "Expires": "0",
            "Set-Cookie": `password=${PASSWORD}; Path=/; Max-Age=3600; HttpOnly; Secure` // Cookie sécurisé pour 1 heure
          },
        });
        return response;
      } else {
        return returnLoginPage(ip, 'Incorrect password. Please try again.');
      }
    }
