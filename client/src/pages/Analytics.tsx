  ...
  if (isLoading) {
    return (
      <div className="dot-grid-bg min-h-screen flex flex-col items-center justify-center text-white">
        <Loader2 className="animate-spin text-[#00ABFF] mb-4" size={64} />
        <p className="text-gray-400 font-bold animate-pulse tracking-widest text-xs uppercase">Initializing Dashboard...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="dot-grid-bg min-h-screen flex flex-col items-center justify-center text-white p-6">
        <div className="bg-white/5 border border-white/10 rounded-3xl p-12 max-w-lg w-full text-center space-y-8 backdrop-blur-xl">
          ...
          <div className="space-y-3">
            ...
            <p className="text-gray-400 text-sm font-medium px-4 leading-relaxed">{error}</p>
          </div>
          <button onClick={() => window.location.href = "/"} className="w-full bg-[#00ABFF] text-white font-bold py-5 rounded-2xl hover:bg-[#0099EE] transition-all uppercase tracking-widest text-sm shadow-lg shadow-[#00ABFF]/20">Return to Home</button>
        </div>
      </div>
    );
  }
    ...
    <div className="dot-grid-bg min-h-screen flex flex-col font-sans text-white relative selection:bg-[#00ABFF] selection:text-white">
      ...
      {overlay.type && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 transition-all">
          <div className="bg-[#0a0d14] border border-white/10 rounded-3xl p-10 max-w-lg w-full shadow-2xl space-y-8 relative overflow-hidden">
            <button 
            ...
            <div className="text-center space-y-3">
              <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4 border ${overlay.type === 'add' ? 'bg-green-500/10 border-green-500/20 text-green-500' : 'bg-red-500/10 border-red-500/20 text-red-500'}`}>
            ...
            <div className="space-y-6">
              <input 
                ...
                className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-6 text-center text-5xl font-bold focus:outline-none focus:border-[#00ABFF] transition-all"
                placeholder="0"
              />
              ...
              <div className="grid grid-cols-3 gap-4">
                {["100", "500", "1000"].map(val => (
                  <button 
                    ...
                    className="bg-white/5 hover:bg-white/10 border border-white/10 py-4 rounded-xl text-sm font-bold transition-all"
                  >
                    +{val}
                  </button>
                ))}
            ...
            <button 
              ...
              className={`w-full py-5 rounded-2xl font-bold uppercase tracking-widest text-sm transition-all flex items-center justify-center gap-3 ${
                overlay.type === 'add' 
      ...
      <main className="flex-1 p-8 pt-32 max-w-[1400px] mx-auto w-full space-y-12 pb-32">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs font-bold uppercase tracking-[0.2em] text-[#00ABFF]">System Analytics</span>
            </div>
            <h1 className="text-6xl font-extrabold tracking-tighter uppercase flex items-center gap-4">
          ...
          <div className="flex items-center gap-6">
            <div className="flex bg-white/5 p-1.5 rounded-2xl border border-white/10 shadow-xl">
              <button onClick={() => setOverlay({ type: 'add', amount: "100" })} className="p-3 text-green-500 hover:bg-green-500/10 rounded-xl transition-all"><Plus size={28} /></button>
              <div className="w-px h-8 bg-white/10 my-auto mx-2"></div>
              <button onClick={() => setOverlay({ type: 'remove', amount: "100" })} className="p-3 text-red-500 hover:bg-red-500/10 rounded-xl transition-all"><Minus size={28} /></button>
            </div>
            <div className="flex flex-col items-end">
              <div className="text-xs text-[#00ABFF] font-bold uppercase tracking-widest mb-1">Live Sync</div>
              <div className="text-sm text-gray-400 font-mono bg-white/5 px-4 py-2 rounded-xl border border-white/10">{lastUpdated.toLocaleTimeString()}</div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            ...
            <div key={i} className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4 hover:border-[#00ABFF]/30 transition-all group shadow-xl">
        ...
        {/* Graph Section */}
        <div className="bg-white/5 border border-white/10 rounded-[2rem] p-8 space-y-8 shadow-2xl">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-[#00ABFF]/10 rounded-2xl border border-[#00ABFF]/20">
                <Activity size={28} className="text-[#00ABFF]" />
              </div>
              <div>
                <h2 className="text-2xl font-bold uppercase tracking-tight">Visitor <span className="text-[#00ABFF]">Trend</span></h2>
                <p className="text-xs text-gray-500 font-bold uppercase tracking-widest">Last 7 days unique sessions</p>
              </div>
            </div>
          </div>
          ...
          <div className="h-[450px] w-full pt-6">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data?.dailyStats} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                <defs>
                ...
                />
                <Tooltip 
                  contentStyle={{ backgroundColor: '#0a0d14', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', fontSize: '14px', fontWeight: 'bold', padding: '12px' }} 
                  itemStyle={{ color: '#00ABFF' }}
                  cursor={{ stroke: '#00ABFF', strokeWidth: 2 }}
                />
                <Area type="monotone" dataKey="views" stroke="#00ABFF" strokeWidth={5} fillOpacity={1} fill="url(#colorViews)" animationDuration={1500} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="p-8 border-b border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 font-bold uppercase tracking-widest">Real-time deep-packet log</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 bg-white/5 p-2 rounded-2xl border border-white/10">
              <button 
                ...
                className="p-2.5 hover:bg-white/10 disabled:opacity-20 rounded-xl transition-all"
              >
                <ChevronLeft size={24} />
              </button>
              <div className="flex items-center gap-2 px-4 text-xs font-bold border-x border-white/10">
                <span className="text-gray-500">PAGE</span>
                <span className="text-[#00ABFF] text-lg">{currentPage}</span>
                <span className="text-gray-500">/</span>
                <span className="text-lg">{data?.pagination.totalPages || 1}</span>
              </div>
              <button 
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-white/5 text-gray-500 font-bold uppercase text-xs tracking-[0.2em]">
                  <th className="px-10 py-6">Visitor Identifier</th>
                  <th className="px-10 py-6">Resource Path</th>
              ...
              <tbody className="divide-y divide-white/5">
                {data?.recentVisits.map((visit, i) => (
                  <tr key={i} className="hover:bg-white/[0.03] transition-all group">
                    <td className="px-10 py-8 font-mono text-[#00ABFF] text-base group-hover:translate-x-2 transition-transform">
                      {visit.ip_address}
                    </td>
                    <td className="px-10 py-8">
                      <span className="bg-[#00ABFF]/10 px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-widest text-[#00ABFF] border border-[#00ABFF]/20">
                        {visit.path}
                      </span>
                    </td>
                ...
                {(!data?.recentVisits || data.recentVisits.length === 0) && (
                  <tr>
                    <td colSpan={3} className="px-10 py-20 text-center text-gray-600 font-bold uppercase tracking-[0.3em] text-sm">
          ...
          <div className="p-8 border-t border-white/10 flex flex-col sm:flex-row justify-between items-center gap-6">
            <div className="flex items-center gap-3">
              ...
              <p className="text-xs text-gray-600 font-bold uppercase tracking-widest">
                Total Log Entries: <span className="text-gray-400 ml-2 text-sm">{data?.pagination.totalItems.toLocaleString()}</span>
              </p>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <input 
                ...
                className="bg-white/5 border border-white/10 rounded-xl px-5 py-3 text-sm font-bold w-24 text-center focus:outline-none focus:border-[#00ABFF] transition-all"
                placeholder="0"
              />
            </div>
          </div>
